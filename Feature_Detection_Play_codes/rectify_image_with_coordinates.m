...
iP = world2img(world_point', tform, cameraParams.Intrinsics );
iW = img2world2d(iP, tform, cameraParams.Intrinsics);

iW_bounds = img2world2d(camera_bound, tform, cameraParams.Intrinsics);
%%
...
yaw=289+90
pitch = 20;
 [UVd,flag] = xyz2DistUV(intrinsics,[-100, -10, 67, yaw, pitch, 0], world_point'); world_UV = reshape(UVd,2,[]);
%
[xyz] = distUV2XYZ(intrinsics,[-100, -10, 67, yaw, pitch, 0],[camera_bound; world_UV]','z',[0 0 0 0 0 0 0])

[x,y]=meshgrid([1:3840], [1:2160]);
[xyz_image] = distUV2XYZ(intrinsics,[-100, -10, 67, yaw, pitch, 0],[x(:) y(:)]','z', zeros(1, size(x(:),1)));

camera_point = [-100, -10 67]
camera_angle = [-180-90+pitch+90 0 270-yaw] % the smaller the additional value, the closer to nadir
R = eul2rotm(deg2rad(camera_angle), 'XYZ');
% image
...
[Ir]= imageRectifier(I,intrinsics,[-10, -100, 70, yaw, pitch, 0],iX,iY,zeros(size(iX,1), size(iX,2)),0);
