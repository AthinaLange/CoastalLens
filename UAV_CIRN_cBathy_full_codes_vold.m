...
for nn = 1:num_flights
    ...
    imageDirectory = 'images_10Hz/';
    %if ~exist(imageDirectory) % extract 10Hz images
        mkdir(imageDirectory)
        for ii = 1:length(mov_id)
            mkdir(['images_10Hz/' char(string(ii))])
            system(['ffmpeg -i ' char(string(C.FileName(mov_id(ii)))) ' -qscale:v 2 -r 10 images_10Hz/' char(string(ii)) '/Frame_%04d.jpg'])
        end
        for ii = 1:length(mov_id)
            Ltemp=sort(split(string(ls(imageDirectory)))); Ltemp=Ltemp(2+length(mov_id):end); % Athina: changes
            Lfull = length(Ltemp);
            L=sort(split(string(ls([imageDirectory char(string(ii))])))); L=L(2:end); % Athina: changes
            if ii == 1
                movefile(sprintf('images_10Hz/%d/Frame_*', ii),'images_10Hz/')
            else
                for ll = 1:length(L)
                    if ll < 10
                    ...
                    movefile(sprintf('images_10Hz/%d/Frame_%s.jpg', ii,id),sprintf('images_10Hz/Frame_%s.jpg', char(string(ll+Lfull))))
                end
            end
        end
    
        for ii = 1:length(mov_id); rmdir(sprintf('images_10Hz/%d', ii), 's');end
   % end
    cd('..')
