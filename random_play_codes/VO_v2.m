...
%%
% VISUALIZE CHANGE
R.FullAdjusted_prevFrame = R.FullRate_prevFrame;

for viewId = 2:length(images.Files)
    R.FullAdjusted_prevFrame(viewId).A = R.FullAdjusted_prevFrame(viewId-1).A *  R.FullAdjusted_prevFrame(viewId).A;
   % figure(100); clf; I1= imshowpair(readimage(images, viewId-1),readimage(images, viewId));
    % figure(200); clf; I2=imshowpair(readimage(images, 1), ...
...
plot([R.FullRate_OGFrame.RotationAngle], 'LineWidth', 3)
yline(0)
...
plot([R.FullRate_OGFrame.RotationAngle],'Color', colors(4,:), 'LineWidth', 3)
title('Total change')
ylabel('Deg')
xlabel('Frame')
yline(0)
