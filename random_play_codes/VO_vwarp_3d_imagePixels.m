...
load('/Users/athinalange/Desktop/DATA/Work/UAV_rectification/Athina_Automated_rectification/DATA/20211026_Torrey/01/Processed_data/20211026_Torrey_01_IOEOInitial.mat', 'extrinsics', 'intrinsics', 'worldPose', 'R', 'rot_answer', 'cutoff')
...
load('/Users/athinalange/Desktop/DATA/Work/UAV_rectification/Athina_Automated_rectification/DATA/20211026_Torrey/01/Processed_data/target_gcp.mat')

load('/Users/athinalange/Desktop/DATA/Work/UAV_rectification/Athina_Automated_rectification/CPG_data/products_Torrey.mat')
Products(2:7) = [];
Products(3:8) = [];
%% Blacks
...
if all(abs([R.MinuteRate_OGFrame.RotationAngle]) < 5) % Small Azimuthal Rotation
    ...
    for viewId = 2:length(images.Files)
        ...
        R.FullRate_OGFrame_rotation(viewId) = rotation;
    end %for viewId = 2:length(images.Files)

else % Large Azimuthal Rotation
   
    %R.FullRate_Adjusted_rotation = rigidtform2d;
...
%%
    for viewId = 2:length(images.Files)
        ...
        % R.FullRate_prevFrame_rotation(viewId) = rotation;
        % R.FullRate_Adjusted_rotation(viewId).A = R.FullRate_Adjusted_rotation(viewId-1).A * R.FullRate_prevFrame_rotation(viewId).A;
        % 
        %% Relative to OG
        [currPoints, currFeatures, indexPairs] = helperDetectAndMatchFeatures(ogFeatures, I, cutoff, numPoints, 'On');
        ...
        if length(relPose) ~= 1
            ...
            if length(relPose) ~= 1
                % find projection point that is closest Euclidian distanceto previous origin point
...
end % if all(abs([R.MinuteRate_OGFrame.RotationAngle]) < 5) % Small Azimuthal Rotation
...
%% GET PIXELS
save(['/Users/athinalange/Desktop/DATA/Work/UAV_rectification/Athina_Automated_rectification/DATA/20211026_Torrey/01/Processed_data/R.mat'], 'R')
% VISUALIZE CHANGE
for viewId = [1000 2000 6000 9000]%2:length(images.Files)
   figure(100); clf; I1= imshowpair(readimage(images, viewId-1),readimage(images, viewId));
    figure(200); clf; I2=imshowpair(readimage(images, 1), ...
        imwarp(readimage(images, viewId), R.FullRate_Adjusted(viewId), OutputView=imref2d(size(readimage(images, viewId-1)))));
    figure(viewId);clf;
    imshowpair(I1.CData, I2.CData, 'montage')
    title({char(string(viewId)), 'Raw                                                Corrected'})
    pause(0.5)
end
%%
