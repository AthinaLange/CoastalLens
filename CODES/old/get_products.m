...
for dd = 1 : length(day_files)
    ...
    for ff = 2%1 : length(flights)
        ...
        for hh = 1 : length(extract_Hz)
            ...
            if isfile(fullfile(odir, 'Processed_data', [oname '_Products_' char(string(extract_Hz(hh))) 'Hz' ]))
                load(fullfile(odir, 'Processed_data', [oname '_Products_' char(string(extract_Hz(hh))) 'Hz' ]))
            end
            if R.ind_scp_method == 1 % Using Feature Detection/Matching
                %extrinsicsInitial = extrinsics;
                load(fullfile(odir, 'Processed_data', [oname '_IOEO_FD_' char(string(extract_Hz(hh))) 'Hz' ]), 'R')
            ...
            elseif R.ind_scp_method == 2 % SCPs
                %% =========================== SCPs ==================================
                load(fullfile(odir, 'Processed_data', [oname '_IOEO_SCP_' char(string(extract_Hz(hh))) 'Hz' ]), 'R')
                
                for viewId = 1:2450%length(images.Files)
                    viewId
                    I = undistortImage(readimage(images, viewId), R.intrinsics);
                    for pp = 1:length(Products)
            ...
            save(fullfile(odir, 'Processed_data', [oname '_Products_' char(string(extract_Hz(hh))) 'Hz' ]),'Products', '-append')
...
[P, ~, R, IC] = intrinsicsExtrinsics2P(intrinsics, localExtrinsics);
