...
clearvars -except camera_type day_files global_dir *_png DEM
%%
close all
switch camera_type
    case 'ARGUS'
...
for  dd = 1 : length(day_files)
    ...
    for ff = 1:loop_num
        ...
        for pp = ids_xtransect % repeat for all xTransects
            ...
            cBathyCT(pp).ct.x = Products(pp).Eastings;
            cBathyCT(pp).ct.y = Products(pp).Northings;

            cBathyCT(pp).survey.z_interp = (interp1(cBathyCT(pp).survey.x, cBathyCT(pp).survey.z, cBathyCT(pp).ct.x));
            cBathyCT(pp).survey.z_interp_m = flipud(cBathyCT(pp).survey.z_interp);
            ...
            viewId1=find(all([contains(timestack_png.Files, oname), contains(timestack_png.Files, ['y_' char(string(Products(pp).y)) ''])], 2)==1);


            viewId2=find(all([contains(prediction_png.Files, oname), contains(prediction_png.Files, ['y_' char(string(Products(pp).y)) ''])], 2)==1);
            if ~isempty(viewId1) & ~isempty(viewId2)

                timestack = readimage(timestack_png, viewId1);
                prediction = readimage(prediction_png, viewId2);
                
                assert(size(timestack,1)==size(prediction,1) & size(timestack,2)==size(prediction,2),['Error (run_cBathyCT): timestack and prediction wrong dimensions. Please check orientation and dimensions of both.']);
                ...
                if ((length(find(timestack==0))/(size(timestack,1)*size(timestack,2))) < 0.25 && aa(1) < round(2*size(timestack,1)/3)) || aa(1) > round(9.5*size(timestack,1)/10)
                    ...
                    bathy.fCombined.h = (bathy.fCombined.h)-cBathyCT(pp).tide;
                    bathy.fCombined.hErr = (bathy.fCombined.hErr);
                    ...
                    if i ~= 0
                        ...
                        cBathyCT(pp).cbathy.z_interp_m = flipud(cBathyCT(pp).cbathy.z_interp);
                        cBathyCT(pp).cbathy.zerr_interp_m = flipud(cBathyCT(pp).cbathy.zerr_interp);
                        ...
                        % Red dot: Entire wave track broken, dot at beginning of track (NaN)
                        ...
                        switch camera_type
                            case 'UAV'
                                ...
                                aa = find(sum([[cutoff_da > 3*xx]; [cutoff_dc > 3*xx]]',1) ~=0);
                                if isempty(aa)
                                    cutoff.cbathy = NaN;
                                else
...
%%
        ...
        for pp = 1:length(cBathyCT)
            if ~isempty(cBathyCT(pp).composite)
                scatter3(cBathyCT(pp).ct.x,cBathyCT(pp).ct.y, flipud(cBathyCT(pp).cbathy.cbathy_hErr_m), 'r')
