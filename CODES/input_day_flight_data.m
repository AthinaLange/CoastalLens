                ...
                case 'Yes'
                    ...
                    if ~exist('cameraParams', 'var') && (~exist('cameraParams_undistorted', 'var') && ~exist('cameraParams_distorted', 'var'))
                        ...
                        if exist('cameraParams', 'var')
                            ...
                            end %  if class(cameraParams_undistorted) == 'cameraParameters'
                            if class(cameraParams_distorted) == 'cameraParameters'
                                cameraParams_distorted = cameraParams_distorted.Intrinsics;
                            end % if class(cameraParams_distorted) == 'cameraParameters'
    ...
    for ff = 1 : length(flights)
        ...
        for pp = ids_grid % repeat for all grids
            ...
            while gridChangeIndex == 0
                ...
                answer = questdlg('Happy with grid projection?', 'Grid projection', 'Yes', 'No', 'Yes');
                switch answer
                    case 'Yes'
                        gridChangeIndex = 1;
                    case 'No'
        ...
        if ~isempty(find(contains(extractfield(Products, 'type'), 'xTransect'), 1))
            ...
            while gridChangeIndex == 0
                ...
                answer = questdlg('Happy with transect projection?', 'Transect Projection', 'Yes', 'No', 'Yes');
                switch answer
                    case 'Yes'
                        gridChangeIndex = 1;
                    case 'No'
