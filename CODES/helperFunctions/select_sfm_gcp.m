...
answer = questdlg('Do you want to select from SfM or image first?', 'GCP locations', 'Image', 'SfM', 'Image');
switch answer
    case 'Image'
        %% Choose GCP Coordinates on Image
       select_image_gcp
        %% Find corresponding points on SfM
        Points = sfmPoints;
        select_pointcloud_gcp
        sfm_gcp = selectedPoints;
        
    case 'SfM'
        %% Get SfM GCPs
        answer2 = questdlg('Do you have a SfM template for this location? ', 'SfM GCP template', 'Yes', 'No', 'No');
        switch answer2
            case 'Yes'
                disp('Load in SfM gcp template.')
                [temp_file, temp_file_path] = uigetfile(global_dir, 'SfM GCP template');
                load(fullfile(temp_file_path, temp_file)); clear temp_file*
                if ~exist('sfm_gcp', 'var')
                    disp('SfM GCPs not correct.')
                    disp('Select SfM GCPs.')
                    gcp_num = str2double(inputdlg({'How many SfM GCPs do you want to find?'}));
                    select_pointcloud_gcp
                    sfm_gcp = selectedPoints;
                end
            case 'No'
        ...
        % plot SfM gcps
        hLid = figure(2);clf
        scatter3(sfmPoints(:,1), sfmPoints(:,2), sfmPoints(:,3), 20, cPoints, 'filled')
      %  set(gca,'YDir', 'reverse')
        colorbar
        caxis([0 10])
        zlim([0 10])
        xlim([min(sfm_gcp(:,1))-50 max(sfm_gcp(:,1))+50])
        ylim([min(sfm_gcp(:,2))-50 max(sfm_gcp(:,2))+50])
        hold on; % so we can highlight clicked points without clearing the figure
        scatter3(sfm_gcp(:,1), sfm_gcp(:,2), sfm_gcp(:,3)+.2, 100, 'r', 'filled')
        for ii = 1:length(sfm_gcp)
            text(sfm_gcp(ii,1)+1, sfm_gcp(ii,2)+1, sfm_gcp(ii,3)+.4, ['GCP ' char(string(ii))], 'FontSize', 14, 'BackgroundColor', 'w')
        end
        view(-90,90)
