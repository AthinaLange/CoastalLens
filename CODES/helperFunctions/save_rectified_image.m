    ...
    for pp = 1:length(Products)
        if contains(Products(pp).type, 'Grid')
            ...
            for tt = 1:size(Products(pp).Irgb_2d,1) % go through all time steps
                ...
                imwrite(image,fullfile(fullfile(save_dir, 'Grid'), [oname '_' Products(pp).type '_datetime_' t '_xlim_' xlim 'm_ylim_' ylim 'm_dx_' dx 'm_dy_' dy 'm.png']));
        ...
        if contains(Products(pp).type, 'Grid')
            ...
            for tt = 1:size(Products(pp).Irgb_2d,1) % go through all time steps
                ...
                imwrite(image,fullfile(fullfile(save_dir, 'Grid'), ...
                    [oname '_' Products(pp).type '_' char(string(tt)) '.png']));
            end % for tt = 1:size(Products(pp).Irgb_2d,1)
        else
            imwrite(Products(pp).Irgb_2d,fullfile(save_dir, [oname '_' Products(pp).type ' pp=' char(string(pp)) '.png']));
        end % if contains(Products(pp).type, 'Grid')
    end % for pp = 1:length(Products)
