...
answer = questdlg('Do you want to select from LiDAR or image first?', 'GCP locations', 'Image', 'LiDAR', 'Image');
switch answer
    case 'Image'
    ...
    case 'LiDAR'
        %% Get LiDAR GCPs
        answer2 = questdlg('Do you have a LiDAR template for this location? ', 'LiDAR GCP template', 'Yes', 'No', 'No');
        switch answer2
            case 'Yes'
                disp('Load in LiDAR gcp template.')
                [temp_file, temp_file_path] = uigetfile(global_dir, 'LiDAR GCP template');
                load(fullfile(temp_file_path, temp_file)); clear temp_file*
                if ~exist('lidar_gcp', 'var')
                    disp('LiDAR GCPs not correct.')
                    disp('Select LiDAR GCPs.')
                    gcp_num = str2double(inputdlg({'How many LiDAR GCPs do you want to find?'}));
                    select_pointcloud_gcp
                    lidar_gcp = selectedPoints;
                end
            case 'No'
        ...
        % plot LiDAR gcps
        hLid = figure(2);clf
        scatter3(lidarPoints(:,1), lidarPoints(:,2), lidarPoints(:,3), 20, cPoints, 'filled')
        %set(gca,'YDir', 'reverse')
        colorbar
        caxis([0 20])
        zlim([0 20])
        xlim([min(lidar_gcp(:,1))-50 max(lidar_gcp(:,1))+50])
        ylim([min(lidar_gcp(:,2))-50 max(lidar_gcp(:,2))+50])
        hold on; % so we can highlight clicked points without clearing the figure
        scatter3(lidar_gcp(:,1), lidar_gcp(:,2), lidar_gcp(:,3)+.2, 100, 'r', 'filled')
        for ii = 1:length(lidar_gcp)
            text(lidar_gcp(ii,1)+1, lidar_gcp(ii,2)+1, lidar_gcp(ii,3)+.4, ['GCP ' char(string(ii))], 'FontSize', 14, 'BackgroundColor', 'w')
        end
        view(-90,90)
