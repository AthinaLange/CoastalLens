...
iopath = [odir '/Processed_data/' oname '_IO.mat'];
...
gcpUvdPath = [odir '/Processed_data/' oname '_gcpUVdInitial.mat'];
...
gcpXyzPath = [odir '/gps_northings.txt'];
...
UVd=reshape([gcp(gcpInd).UVdo],2,length(x))'; % N x 2 matrix with rows=gcps, columns= U,V
...
for k=1:length(gcp)
    % Clicked Values
    h1=plot(gcp(k).UVdo(1),gcp(k).UVdo(2),'ro','markersize',10,'linewidth',3);
    text(gcp(k).UVdo(1)+30,gcp(k).UVdo(2),num2str(gcp(k).num),'color','r','fontweight','bold','fontsize',15)
...
for k=1:length(gcp)
    ...
    [xyzReproj(k,:)] = distUV2XYZ(intrinsics,extrinsics,[gcp(k).UVdo(1); gcp(k).UVdo(2)],'z',gcp(k).z);
